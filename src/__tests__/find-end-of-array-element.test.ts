...
describe("findEndOfArrayElement(haystack, arrayElementCharacterIndex)", () => {
  describe("with well-formed array strings", () => {
    it("matches primitive values correctly", () => {
      ...
      expect(
        findEndOfArrayElement('["foo","barrington","bazzzuuuuuuor"]', 1),
      ).toEqual(6);
      expect('["foo","barrington","bazzzuuuuuuor"]'.substring(1, 6)).toEqual(
        '"foo"',
      );
      expect(
        findEndOfArrayElement('["foo","barrington","bazzzuuuuuuor"]', 7),
      ).toEqual(19);
      expect('["foo","barrington","bazzzuuuuuuor"]'.substring(7, 19)).toEqual(
        '"barrington"',
      );
      expect(
        findEndOfArrayElement('["foo","barrington","bazzzuuuuuuor"]', 20),
      ).toEqual(35);
      expect('["foo","barrington","bazzzuuuuuuor"]'.substring(20, 35)).toEqual(
        '"bazzzuuuuuuor"',
      );
    });
    ...
    it("doesn't get confused by parantheses nested in strings", () => {
      ...
      expect(findEndOfArrayElement('["[\\"foo\\"]","[\\"foo\\"]"]', 1)).toEqual(
        12,
      );
      expect('["[\\"foo\\"]","[\\"foo\\"]"]'.substring(1, 12)).toEqual(
        '"[\\"foo\\"]"',
      );
      expect(
        findEndOfArrayElement('["[\\"foo\\"]","[\\"foo\\"]"]', 13),
      ).toEqual(24);
      expect('["[\\"foo\\"]","[\\"foo\\"]"]'.substring(13, 24)).toEqual(
        '"[\\"foo\\"]"',
      );
    });
    ...
    it("correctly matches nested objects", () => {
      expect(
        findEndOfArrayElement('["foo",{"bar": true, "baz": false}]', 1),
      ).toEqual(6);
      expect('["foo",{"bar": true, "baz": false}]'.substring(1, 6)).toEqual(
        '"foo"',
      );
      expect(
        findEndOfArrayElement('["foo",{"bar": true, "baz": false}]', 7),
      ).toEqual(34);
      expect('["foo",{"bar": true, "baz": false}]'.substring(7, 34)).toEqual(
        '{"bar": true, "baz": false}',
      );
    });
  ...
  describe("with malformed array strings", () => {
    it("handles empty arrays correctly", () => {
      expect(findEndOfArrayElement("[]", 0)).toEqual(
        FIND_END_OF_ARRAY_ELEMENT_INDEX_INVALID_CHARACTER,
      );
    });

    it("handles non-terminated arrays correctly", () => {
      expect(findEndOfArrayElement("[true,false", 7)).toEqual(
        FIND_END_OF_ARRAY_ELEMENT_INDEX_INVALID_CHARACTER,
      );
    });
  });
