...
export function findArrayElementIndices(haystack: string): [start: number, end: number][] {
    if (haystack.at(0) !== "[") {
        throw new Error(`findArrayElementIndices(haystack): Expected haystack to begin with "[", got: "${haystack.at(0)}"`)
    }
    ...
    while (true) {
        ...
        if (typeof lastElementIndices === "undefined") {
            ...
            if (index === FIND_END_OF_ARRAY_ELEMENT_INDEX_INVALID_CHARACTER) {
                throw new Error(`Expected to find end for all elements, got -1 for element #${elementIndices.length}`)
            }
