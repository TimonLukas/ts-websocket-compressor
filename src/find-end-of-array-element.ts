...
export function findEndOfArrayElement(
  haystack: string,
  arrayElementCharacterIndex: number,
): number {
  if (arrayElementCharacterIndex < 0) {
    throw new Error(
      `Expected findEndOfArrayElement(haystack, arrayElementCharacterIndex) => arrayElementCharacterIndex == ${arrayElementCharacterIndex} > 0`,
    )
  }

  if (arrayElementCharacterIndex >= haystack.length) {
    throw new Error(
      `Expected findEndOfArrayElement(haystack, arrayElementCharacterIndex) => arrayElementCharacterIndex == ${arrayElementCharacterIndex} <= haystack.length == ${haystack.length}`,
    )
  }
