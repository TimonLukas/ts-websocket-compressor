...
export function findMatchingCharacterIndex(haystack: string, openingCharacterIndex: number): number {
    if (openingCharacterIndex < 0) {
        throw new Error(`Expected findMatchingCharacterIndex(haystack, openingCharacterIndex) => openingCharacterIndex == ${openingCharacterIndex} > 0`)
    }

    if (openingCharacterIndex >= haystack.length) {
        throw new Error(`Expected findMatchingCharacterIndex(haystack, openingCharacterIndex) => openingCharacterIndex == ${openingCharacterIndex} <= haystack.length == ${haystack.length}`)
    }
