...
export class MessageCompressor {
    ...
    private compressRegisteredMessage(message: Record<string, unknown>, id: number): string {
        ...
        if (typeof keysSet === "undefined") {
            throw new Error("MessageCompressor::compressRegisteredMessage(message) => keySet is undefined?")
        }
    ...
    decompress(message: string): Record<string, unknown> | null {
        ...
        if (messageTypeId === -1) {
            throw new Error(`MessageCompressor::uncompress got unexpected messageTypeId == -1, start of message: ${message.substring(0, 50)}`)
        }
        ...
        return this.decompressRegisteredMessage(message.substring(messageTypeIdString.length), messageTypeId)
    }

    private decompressGeneralMessage(message: string): Record<string, unknown> {
        const indices = findArrayElementIndices(message)
        if (indices.length % 2 !== 0) {
            throw new Error(`MessageCompressor::decompressGeneralMessage(message) => expected indices to have even count, found uneven: ${indices.length}`)
        }
        ...
        for (let keyIndex = 0; keyIndex < indices.length - 1; keyIndex += 2) {
            ...
            if (typeof keyIndices === "undefined") {
                throw new Error(`MessageCompressor::decompressGeneralMessage(message) => couldn't find keyIndex == ${keyIndex} in indices`)
            }
            if (typeof valueIndices === "undefined") {
                throw new Error(`MessageCompressor::decompressGeneralMessage(message) => couldn't find valueIndex == ${valueIndex} in indices`)
            }
    ...
    private decompressRegisteredMessage(message: string, id: number): Record<string, unknown> {
        const keysSet = this.registeredMessageTypes.get(id)

        if (typeof keysSet === "undefined") {
            throw new Error(`MessageCompressor::uncompressRegisteredMessage(message, id) got unexpected id ${id}`)
        }

        const indices = findArrayElementIndices(message)
        const values = indices.map(([start, end]) => message.substring(start, end).trim())
        return Object.fromEntries([...keysSet].map((key, index) => {
            const value = values[index]

            if (typeof value === "undefined") {
                throw new Error(`MessageCompressor::decompressRegisteredMessage(message, id) => couldn't find value with index ${index}`)
            }
    ...
    getMessageTypeId(message: Record<string, unknown>): number {
        ...
        const registeredMessageId = this.registeredMessageTypeKeysToIndex.get(joinedKey)
    ...
    handleDictionaryUpdates(dictionary: [Record<number, string[]>, Record<string, number>]): void {
        const [registeredMessageTypes, registeredGeneralKeysToIds] = dictionary

        const parsedRegisteredMessageTypes = Object.entries(registeredMessageTypes)
        ...
        this.registeredGeneralKeysToIds = new Map(Object.entries(registeredGeneralKeysToIds))
    ...
    private eventHandlers: {
        "send-dictionary-updates-to-clients": ((dictionary: [Record<string, string[]>, Record<string, number>]) => void)[]
    } = {
        "send-dictionary-updates-to-clients": []
    }

    on<K extends keyof typeof this.eventHandlers, T extends ((typeof this.eventHandlers)[K])[number]>(event: K, handler: T): StopHandle {
    ...
    private emit<K extends keyof typeof this.eventHandlers, T extends ((typeof this.eventHandlers)[K])[number]>(event: K, args: Parameters<T>) {
    ...
    private emitSendDictionaryUpdatesToClients(): void {
        const registeredMessageTypes: Record<string, string[]> = mapToRecord(this.registeredMessageTypes, setToArray)
        const registeredGeneralKeysToIds: Record<string, number> = mapToRecord(this.registeredGeneralKeysToIds)

        this.emit("send-dictionary-updates-to-clients", [[registeredMessageTypes, registeredGeneralKeysToIds]])
    }
    ...
    private queueEmitSendDictionaryUpdatesToClients(): void {
        ...
        this.timeoutEmit = setTimeout(() => this.emitSendDictionaryUpdatesToClients(), this.timeoutEmitQueueInMs)
    }
}
